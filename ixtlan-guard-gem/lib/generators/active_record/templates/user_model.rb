require 'ixtlan/models/user_update_manager'
...
<% end -%>
  ...
  def self.update_manager
    @manager ||= Ixtlan::Models::UserUpdateManager.new( :group_model => <%= group_class_name %>, :user_id => :<%= user_name %>_id, :group_id => :<%= group_name %>_id, :plural_group_name => :<%= plural_group_name %> )
  end
